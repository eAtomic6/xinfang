...
import { isProjectPrice, isIdCard } from '@/views/login/validate.js'
...
let arr = []
let arr2 = []
let commissions = [] //分佣的数据
let commissionseId = [] //分佣的数据
let commissionsdepId = [] //分佣的数据
...
export default {
    ...
    data() {
        return {
            ...
            settlementRulesRenChou: {
                ...
                //手机号码
                custMobile: [{ required: true, trigger: 'blur' }],
                //身份证号
                custIdCard: [{ required: true, trigger: 'blur' }],
                //认筹金额//认购金额
                dealPay: [
                    {
                        required: true,
                ...
                //房屋房号
                houseNo: [{ trigger: 'blur' }],
                // //房屋面积
                houseArea: [{ trigger: 'blur' }],
                //房屋总价
                housePrice: [
                ...
                //成交人
                dealByName: [{ required: true, trigger: 'blur,change' }],
                //应收佣金
                expectedCommision: [
                    {
                        required: true,
            ...
            settlementRules: {
                ...
                //房屋房号
                houseNo: [{ required: true, trigger: 'blur' }],
                //房屋面积
                houseArea: [{ required: true, type: 'number', trigger: 'blur' }],
                //房屋总价
                housePrice: [
                ...
                //成交人
                dealByName: [{ required: true, trigger: 'blur' }],
                //应收佣金
                expectedCommision: [
                    {
                        required: true,
            ...
            commissionRules: {
                //分佣比例
                eCharacter: [{ required: true, trigger: 'blur' }],
                eName: [{ required: true, trigger: 'blur' }],
                depName: [{ required: true, trigger: 'blur' }],
                rate: [{ required: true, trigger: 'blur', type: 'number' }]
            },
            // operateTime:[],
            // rengoutime: "", //认购时间
            // fenyong: "", //分佣比例
    ...
    methods: {
        ...
        getList() {
            ...
            contractManagement({ params: params }).then(res => {
                console.log(res)
        ...
        // table列表点击获取数据666
        getListClick(htType) {
            const params = { dealId: this.tableDataId }
            contractManagementClick({ params: params }).then(res => {
                if (res.status === 200) {
                    console.log(res)
            ...
            axios
                ...
                .then(res => {
                    if (res.status === 200) {
                        ...
                        const params = { dealId: this.tableDataId }
                        // 结算详情数据
                        settlementDetailsClick({ params: params }).then(res => {
                            // console.log("结算详情的数据");
                            // console.log(res);
                            // debugger
        ...
        getCommissionAjax() {
            ...
            axios
                .get('/api/deal/commissionDetails', { params: param })
                .then(res => {
                    res = res.data
                    if (res.status === 200) {
        ...
        rowDblclickCon() {
            ...
            getContractInfo({ params: params }).then(res => {
                if (res.status === 200) {
                    console.log(res)

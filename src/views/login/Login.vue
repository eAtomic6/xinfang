...
<div class="login" :style="{background:url_back_img}">
  <div class="login_container">
    ...
    <div class="erweima" v-show="isShowEWM">
      ...
      <p class="textSaoyisao" v-show="isShowWord">请使用<span>新房APP</span>扫一扫登录</p>
      <p v-show="isShowSuccess">扫描成功，请在手机上点击确认</p>
      <el-button type="primary  " 
      @click.native.prevent="backLogin"
      class="loginBtn">返回</el-button>
    </div>
    <div v-show="isShowLogin" class="showLogin">
    <el-form-item  prop="mobile" class="iptPhone">
      <span class="icon icon-yonghuming phoneIcon"></span>
      <el-input v-model="ruleForm.mobile" placeholder="请输入手机号" maxlength=11 class="loginPhone" @change="loginPhoneChange"></el-input>
    </el-form-item>
     <el-form-item prop="captcha">
       <span class="icon icon-mima phonePassword"></span>
      <el-input v-model="ruleForm.captcha" placeholder="请输入短信验证码" class="codeIpt" @keyup.enter.native="loginEnter('ruleForm')"></el-input>
    ...
    <el-button type="text" class="getEWMBtn" @click="showEWM" style="display: none">二维码登录</el-button>
    </div>
  </el-form>
  </div>
</div>
...
export default {
  data() {
    return {
      ...
      url_back_img:"url("+require('../../assets/images/login-banner.jpg')+")"
    };
  },
  created() {
   this.reloginMethod()
  ...
  .login_container {
    ...
    .login_form {
      ...
      .loginTitle {
        ...
        line-height: 63px;
        margin-bottom: 55px;
        background: #f6f6f6 url(../../assets/images/login-border.png) repeat-x bottom center;
      }
      .getEWMBtn {
        position: absolute;
        margin-top: 28px;
      ...
      .loginPhone {
        &::before {
          ...
          width: 9px;
          height: 22px;
          background-image: url(../../assets/images/login-dash.png);
        }
      }
      .getPhoneCd {
        color: $btn;
